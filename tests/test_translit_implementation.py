...
def test_lr_scheduler():
    ...
    sch = LrScheduler(100, type=lrs_type, warmup_steps_part=warmup_steps_part,
                      lr_peak=lr_peak)
    assert abs(sch.learning_rate(step=5) - 15e-5) < 1e-6
    assert abs(sch.learning_rate(step=10) - 3e-4) < 1e-6
    assert abs(sch.learning_rate(step=50) - 166e-6) < 1e-6
    assert abs(sch.learning_rate(step=100) - 0.) < 1e-6
    print('Test is passed!')
